...
/// </summary>
public partial class PackingSlipDocument : PdfDocument<PackingSlipSource>
{

    #region Ctor

    public PackingSlipDocument(PackingSlipSource packingListSource, ILocalizationService localizationService) : base(packingListSource, localizationService)
    {
    }
    ...
    protected void ComposeBillingAddress(IContainer container)
    {
        container.Column(column =>
        {
            ...
            column.Item()
                ...
                .Text(t => ComposeLabel<PackingSlipSource>(t, x => x.BillingAddress));
    ...
    protected void ComposeShippingAddress(IContainer container)
    {
        container.Column(column =>
        {
            ...
            column.Item()
                //.BorderBottom(1)
                .DefaultTextStyle(style => style.SemiBold())
                .Text(t => ComposeLabel<PackingSlipSource>(t, x => x.ShippingAddress));
    ...
    {
        container.ShowOnce().DefaultTextStyle(tStyle => tStyle.SemiBold()).Column(column =>
        {
            column.Item().Row(row =>
            {
                var logoContainer = row.ConstantItem(95).Height(50);
                ...
                row.RelativeItem().AlignRight().Column(column =>
                {
                    ...
                    column.Item().AlignRight().Text(t =>
                        ComposeField(t,
                            Source,
                            x => x.OrderDateUser,
                            date => date.ToString("M/d/yyyy", new CultureInfo(Source.Language.LanguageCulture)),
                            string.Empty,
                            false
                        ));
                });
    ...
    protected void ComposeProducts(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Header(header =>
            {
                header.Cell().Element(cellStyle).AlignLeft().Text(t => ComposeLabel<ProductItem>(t, x => x.Quantity));
            ...
            {
                table.Cell().Element(cellStyle).AlignLeft().Text(product.Quantity);
    ...
    {
        column.Item().Text(t => ComposeField(t, address, x => x.Name, delimiter: string.Empty, displayLabel: false));
        column.Item().Text(t => ComposeField(t, address, x => x.Company, delimiter: string.Empty, displayLabel: false));
        column.Item().Text(t => ComposeField(t, address, x => x.AddressLine, delimiter: string.Empty, displayLabel: false));
        column.Item().Text(t => ComposeField(t, address, x => x.Phone, delimiter: string.Empty, displayLabel: false));
        ...
        column.Item().PaddingVertical(10).Text(t => ComposeField(t, address, x => x.ShippingMethod, delimiter: ": "));
    ...
    protected void ComposeOrderNotes(IContainer container)
    {
        ...
        container.Background(Colors.Grey.Lighten3).Padding(10).Column(column =>
        {
            ...
            column.Item()
                ...
                .DefaultTextStyle(style => style.SemiBold())
                .Text(t => ComposeLabel<PackingSlipSource>(t, x => x.OrderNotes));
